...
public class UserServiceImpl implements UserService {
	...
	public void showUserMenu() {
		...
		System.out.println();
		System.out.println(
				"---------------------------------------------->> WELCOME USER TO My ELECTRONICS <<--------------------------------------------------------");
		System.out.println();
		...
		do {
			...
			case 1:
				...
				for (Product pList : products) {
					...
					System.out.println("\t" + pList.getProductId() + " \t\t" + pList.getProductName() + "\t\t\t"
							+ pList.getProductDescription() + "\t\t\t" + pList.getProductQuantity() + "\t\t\t"
							+ pList.getProductPrice());
					System.out.println(
							"------------------------------------------------------------------------------------------------------------------------------------------");
				}
				...
				while (rs.next()) {
					...
					System.out.println(
							"" + rs.getLong("id") + "\t" + rs.getString("name") + "\t" + rs.getInt("Quantity") + "\t"
									+ rs.getDouble("price") + "\t\t" + (rs.getInt("Quantity") * rs.getDouble("price")));
	...
	public void getOrderDetails(String contactNo) {
		...
		try {
			...
			while (rs2.next()) {
				System.out.println("Order ID: " + rs2.getInt("order_id") + "\t\t Total Amount: "
						+ Math.round(rs2.getDouble("total")) + "\t\t Order Date: " + rs2.getDate("created_at")
						+ "\t\t Payment Status: " + rs2.getString("payment_status"));
			}
